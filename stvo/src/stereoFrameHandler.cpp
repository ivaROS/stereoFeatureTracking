...
void StereoFrameHandler::insertStereoPair(const Mat img_l_, const Mat img_r_ , const int idx_)
{
    ...
    double t_feature_matching = timer_.stop();
    timer_.start();
    
    f2fTracking();
    
    double t_tracking = timer_.stop();
    
#ifdef SHOW_TIME
    std::cout << "Features matching totally took: " << t_feature_matching << " ms." << std::endl;
    std::cout << "Tracking totally took: " << t_tracking << " ms." << std::endl;
#endif
}
...
void StereoFrameHandler::matchF2FPoints()
{
    ...
    for (int i1 = 0; i1 < matches_12.size(); ++i1) {
        ...
        // if (abs(curr_frame->stereo_pt[i2]->pl(0) - prev_frame->stereo_pt[i1]->pl(0)) > 30 || 
        //     abs(curr_frame->stereo_pt[i2]->pl(1) - prev_frame->stereo_pt[i1]->pl(1)) > 30)
        //     continue;
        
        PointFeature* point = prev_frame->stereo_pt[i1];
        point->pl_obs = curr_frame->stereo_pt[i2]->pl;
        point->inlier = true;
        matched_pt.push_back( point );
    ...
    num_tracked_pt = num_tracked_iter;
}
...
void StereoFrameHandler::optimizePose()
{
    ...
    if( n_inliers >= Config::minFeatures() )
    {
        // optimize
#ifdef OPT_COUT
        cout << "[StVO] start optimize pose." << endl;
#endif
        DT_ = DT;
        ...
        {
#ifdef OPT_COUT
            cout << "[StVO] remove outliers." << endl;
#endif
            removeOutliers(DT_);
            ...
            {
#ifdef OPT_COUT
                cout << "[StVO] refine without outliers." << endl;
#endif
    ...
    {
#ifdef OPT_COUT
        cout << "[StVO] set estimated pose." << endl;
#endif
...
void StereoFrameHandler::plotLeftPair(Mat& tracked_img) {
    ...
    im1 = prev_frame->img_l.clone();
    im2 = curr_frame->img_l.clone();
