...
Config::Config()
{
    ...
    max_disp          = 50.0;      // max. disparity (avoid points that are too close to robot)
...
void Config::loadFromFile( const string &config_file )
{
    ...
    Config::maxDisp() = loadSafe(config, "max_disp", Config::maxDisp());
    ...
    Config::useOrbslam2Extractor() = loadSafe(config, "use_orbslam2_extractor", Config::useOrbslam2Extractor());
    Config::iniThFAST() = loadSafe(config, "ini_th_fast", Config::iniThFAST());
    Config::minThFAST() = loadSafe(config, "min_th_fast", Config::minThFAST());
