...
// void PinholeStereoCamera::fromCameraInfo( const sensor_msgs::CameraInfo &left_msg, const sensor_msgs::CameraInfo &right_msg )
// {
//     // For using ROS, we assume the input image pairs are rectified and undistorted.
//     // So no need to compute Q, undistmap
//     if(left_msg.width == right_msg.width && left_msg.height == right_msg.height)
//     {
//         width = left_msg.width;
//         height = left_msg.height;
//     }
//     else
//         throw std::runtime_error("Left and right image sizes are not same.");
// 
//     vector<double> Kl_ = left_msg.K;
//     vector<double> Kr_ = right_msg.K;
//     Kl = vec2mat<double>(Kl_, 3, 3);
//     Kr = vec2mat<double>(Kr_, 3, 3);
// 
//     vector<double> Dl_ = left_msg.D;
//     vector<double> Dr_ = right_msg.D;
//     Dl = vec2mat<double>(Dl_, 1, 5);
//     Dr = vec2mat<double>(Dr_, 1, 5);
// 
//     vector<double> Rl_ = left_msg.R;
//     vector<double> Rr_ = right_msg.R;
//     Rl = vec2mat<double>(Dl_, 3, 3);
//     Rr = vec2mat<double>(Dr_, 3, 3);
// 
//     vector<double> Pl_ = left_msg.P;
//     vector<double> Pr_ = right_msg.P;
//     Pl = vec2mat<double>(Dl_, 3, 4);
//     Pr = vec2mat<double>(Dr_, 3, 4);
// 
//     fx = Pl_[0];
//     fy = Pl_[5];
//     cx = Pl_[2];
...
cv::Point PinholeStereoCamera::projection(const cv::Point3d &P)
{
    cv::Point uv_unit;
    uv_unit.x = cx + fx * P.x / P.z;
    uv_unit.y = cy + fy * P.y / P.z;
    return uv_unit;
}
