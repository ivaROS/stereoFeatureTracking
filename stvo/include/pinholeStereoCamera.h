...
//     void fromCameraInfo( const sensor_msgs::CameraInfo &left_msg, const sensor_msgs::CameraInfo &right_msg );
    ...
    cv::Point projection(const cv::Point3d &P);
...
template <typename T>
Mat vec2mat(const vector<T> &inputVec, int rows, int cols)
{
    Mat output = Mat_<T>(rows, cols);
    memcpy(output.data, inputVec.data(), inputVec.size()*sizeof(T));
